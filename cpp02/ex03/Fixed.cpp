...
/*   Updated: 2025/09/22 10:52:46 by aarie-c2         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Fixed Fixed::operator-() const {
    return Fixed(-this->toFloat());  // Negate as float and construct a Fixed
}
...
bool Fixed::almostEqual(const Fixed& other, float epsilon) const {
    Fixed diff = *this - other;
    if (diff < Fixed(0))
        diff = -diff;

    // Debug print for difference
    std::cout << "Diff for almostEqual: " << diff.toFloat() << std::endl;

    return diff.toFloat() < epsilon;
}